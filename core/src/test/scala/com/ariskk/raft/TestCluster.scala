...
final class TestCluster[T](nodeRef: Ref[Seq[Raft[T]]], chaos: Boolean) {
  ...
  def getNode(id: NodeId): ZIO[Any, Option[Nothing], Raft[T]] = for {
    nodes <- nodeRef.get
  ...
  private def sendMessage(m: Message): ZIO[Any, Option[Nothing], Unit] = for {
    node <- getNode(m.to)
  ...
  def run: ZIO[Any with Clock, Serializable, (Unit, Nothing)] = {
  ...
  def submitCommand(command: WriteCommand): ZIO[Clock, Serializable, Unit] = for {
    ...
    leaderId = ids.zip(states).collectFirst { case (id, state) if state == NodeState.Leader => id }
    _ <- ZIO.fromOption(leaderId).flatMap(id => getNode(id).flatMap(_.submitCommand(command)).unit)
  } yield ()

  def getAllLogEntries: ZIO[Any, RaftException.StorageException, (Seq[NodeId], Seq[List[LogEntry]])] = for {
    nodes <- getNodes
    ids = nodes.map(_.nodeId)
