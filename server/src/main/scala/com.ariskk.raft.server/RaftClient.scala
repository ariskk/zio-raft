...
final class RaftClient(
  ...
  def channel(address: InetAddress, port: Int) = AsynchronousSocketChannel().mapM { client =>
    for {
      address <- SocketAddress.inetSocketAddress(address, port)
      _ <- client
        .connect(address)
  ...
  def submitCommand(command: WriteCommand): ZIO[Clock, Exception, Unit] = for {
    ...
    _ <- response match {
      ...
      case Redirect(leaderId) =>
        for {
          newLeader <- ZIO
            .fromOption(nodes.filter(_.nodeId == leaderId).headOption)
